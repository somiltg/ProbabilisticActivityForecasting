...
class RNN_apm(nn.Module):
    def __init__(self, input_size, output_size, hidden_dim, max_layer_size=30):
        ...
        for i in range(1, max_layer_size + 1):
            self.rnn_cells[i] = nn.RNNCell(input_size, hidden_dim)
    ...
    def forward(self, x):
        ...
        for i in range(1, batch_size + 1):
            ...
            h_out = self.rnn_cells[i].forward(x_cell, hidden[i - 1])
            hidden.append(h_out)
...
class activity_prediction_model:
    ...
    def __init__(self, lr=0.001, reg=0.01, hidden_size=50):
        self.lr = lr
        self.reg = reg
        self.rnn_model: RNN_apm = RNN_apm(211, 5, hidden_size)
    ...
    def fit(self, df, mode='train', train_batch_per_user=3):
        ...
        """
        self.load_model()
        optimizer = optim.Adam(self.rnn_model.parameters(), lr=self.lr, weight_decay=self.reg)
        if mode is 'train':
    ...
    def iteration_construct(self, df, iteration_fn, num_batch_per_user, debug_low=False, debug_high=True):
        ...
        def sample(df, iter, batch_size):
            timeIndexes = df.index.get_level_values(1)
            return df.iloc[iter * batch_size:min((iter + 1) * batch_size, timeIndexes.shape[0])]
    ...
    def get_params(self):
        return self.rnn_model.state_dict()

    def set_params(self, params):
        self.rnn_model.load_state_dict(params)
        self.rnn_model.eval()
    ...
    def cal_class_prob(self, df, t):
        ...
        p_pos = self.rnn_model.forward(x.float())
    ...
    def save_model(self):
        ...
        Arguments:
            None
        """
        torch.save(self.get_params(), 'code/rnn_model.pt')
    ...
    def load_model(self):
        ...
        Arguments:
            None
        """
        if os.stat('code/rnn_model.pt').st_size == 0:
            return
        params = torch.load('code/rnn_model.pt')
